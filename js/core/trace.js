...
(function (root) {
  ...
  const DEFAULTS = {
    ...
    arcMinSweepDeg: 16,
    arrowRadius: 16,        // px from a stroke's end within which a head is its own
    arrowMaxArea: 400
  };
  ...
  function traceFigure(gray, w, h, options) {
    ...
    for (let i = 0; i < segs.length; i++) {
      ...
      if (!line) continue;
      const s = segs[i];
      const headNear = (x, y) => parts.some(p => !used.has(p) &&
        p.comp.area <= o.arrowMaxArea &&
        Math.hypot((p.comp.x0 + p.comp.x1) / 2 - x, (p.comp.y0 + p.comp.y1) / 2 - y) <= o.arrowRadius);
      const startHead = headNear(s.x1, s.y1), endHead = headNear(s.x2, s.y2);
