...
(function (root) {
  ...
  const DEFAULTS = {
    ...
    mergeDistance: 13,      // a hand-drawn stroke wobbles into several Hough peaks
    ...
    arcMinSweepDeg: 16,
    arrowRadius: 13,        // px from a stroke's end within which a head is its own
    arrowMinArea: 8,        // a head straddles the stroke, so removing the stroke splits it
    arrowMinTotalArea: 26,  // the pieces are judged together
    arrowMaxArea: 400,
    arrowMinFill: 0.34      // a head is a solid wedge; an arc or a chevron is a thin curve
  };
  ...
  function traceFigure(gray, w, h, options) {
    ...
    for (let i = 0; i < segs.length; i++) {
      ...
      if (startHead && endHead) { line.kind = 'line'; line.arrows = 'both'; }
      else if (startHead) { line.kind = 'ray'; line.arrows = 'start'; }
      else if (endHead) { line.kind = 'ray'; line.arrows = 'end'; }
    }
